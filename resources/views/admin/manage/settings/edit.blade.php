  ...
  <div class="row justify-content-center">
    ...
    <div class="col-md-12" style="padding-top: 15px; padding-bottom: 15px;">
      <div class="card">
        ...
        <div class="card-body">
          <form method="POST" action="{{ route('settings.edited', ['id' => $settings->id]) }}">
            {{ csrf_field() }}
            <div class="form-group">
              <label for="exampleFormControlSelect1">For (e.g. site_maintanace)</label>
              <input type="text" class="form-control" name="for" id="for" placeholder="For" value="{{$settings->for}}" required>
            </div>
            <div class="form-group">
              <label for="exampleFormControlSelect1">Content (e.g. true)</label>
              <input type="text" class="form-control" name="content" id="content"value="{{$settings->content}}"  placeholder="content" required>
            </div>
            <button style="width: 100%;" class="btn btn-primary btn-animated"><i class="material-icons">edit</i> Save</button>
          </form>
          <form method="POST" action="{{ route('settings.delete', ['id' => $settings->id]) }}">
            {{ csrf_field() }}
